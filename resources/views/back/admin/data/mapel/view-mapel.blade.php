            ...
            <div class="page-title">
                <div class="row">
                    <div class="col-12 col-md-6 order-md-1 order-last">
                        <h3>Data Mata Pelajaran {{$kelas->nama_kelas}}</h3>
                    </div>
                </div>
            </div>
                                ...
                                <div class="card-body">
                                    <div class="card-header" style="display: flex;margin-left: -20px">
                                        <div class="card-head-row">
                                            <a href="{{route("mapel")}}" class="btn btn-warning ml-auto"> <i class="bi bi-arrow-left-circle"></i> Kembali </a>
                                        </div>
                                        <div class="card-head-row" style="margin-left: 10px">
                                            <a href="{{ route('add-mapel', ['kelasId' => $kelas->id]) }}" class="btn btn-info btn=sm ml-auto"> <i class="bi bi-plus-circle" style="margin-right: 4px"></i>Tambah Data</a>
                                        </div>
                                        <div class="card-head-row" style="margin-left: 10px">
                                            <form action="{{route('reset-mapel', ['kelasId' => $kelas->id])}}" method="POST">
                                                @csrf
                                                @method('DELETE')
                                            ...
                                            <div class="card-body" >
                                                <div class="table-responsive" style="margin-left: -20px;margin-top: -20px;">
                                                    <table class="table table-bordered mb-3" id="table1">
                                                        ...
                                                        <tbody>
                                                        @forelse($mapels as $row)
                                                            <tr>
                                                                <td class="text-bold-500">{{ $row->nama }}</td>
                                                                <td class="text-bold-500">{{ $row->guru->nama }}</td>
                                                                <td class="text-bold-500">
                                                                    <a href="{{ route('edit-mapel', ['id' => $row->id, 'kelasId' => $kelas->id]) }}" class="btn icon btn-primary">
                                                                        <i class="bi bi-pencil"></i>
                                                                    </a>
                                                                    <br>
                                                                    <form method="post" action="{{ route('delete-mapel', ['id' => $row->id, 'kelasId' => $kelas->id]) }}" class="d-inline">
                                                                        @csrf
                                                                        @method('DELETE')
